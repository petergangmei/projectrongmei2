...
<div style=" width: 100%;" class="row border-black">
						...
						@foreach($videos as $video)
							<title>{{ $video->post_title }}</title>
							  ...
							  <div class="panel padding-lg" style="width: 100%; float: left; margin-right: 15px;">
								    <div class="panel-">
								    	<video  autoplay preload="metadata" style="border:1px solid skyblue; width: 100%; max-height:400px;"   muted  controls >
									 ...
									 <!-- attributes of video -->
									    <div style="width: 100%; border-bottom:1px double silver;" class="text-right border-black1float-right">
										 ...
										 <div class="border-black text-left">
										 	<!-- subscrib button -->
										 	<button id="sub" style="border:1px solid blue; float:right; float: right; width: 100px; height: 35px; background-color: blue; color: white;">Subscribe
										 	</button>
					...
					@endif
					<!-- div for comment -->
					<div class="comment border-black" id="comments-div">
							...
							<div class="padding-lg" style="border-bottom:0px solid silver; padding-left: ; padding-right: ;">
									...
									<div id="commentscount-div" ><h4><span style="font-size: 1.3em;">{{ $commentsCount->count() }}</span> <samll> Comments</samll></h4>

									</div>
								<br>
								<!-- row for comment div -->
								<div id="loader" class="border-black" >
									<div class="loader">Loading...</div>
								</div>
								
								<div class="row border-black" id="commentsection-div" style="height: 70px;">
									<div class="col-md-1 col-sm-2 text-center ">
										@guest
										<img style="height: 55px; width: 55px; border-radius: 100%; margin: -5px 0px;" class="hidden-xs" src="/storage/default-images/ava.jpg">
										@else
										<?php $userId_path =  auth()->user()->name; ?>
										<img style="height: 55px; width: 55px; border-radius: 100%; margin: -5px 0px;" class="hidden-xs" src="/storage/profile_image/{{ auth()->user()->id }}/{{ auth()->user()->profile_image }}">
										@endguest
										<br>
									</div>
									@guest
						...
						<div class="border-black" id="commentsdisplay-div">
										...
										<div class="col-md-1 col-xs-11">
											...
											<span class="dropdown ">
                                                ...
                                                <ul class="dropdown-menu">
                                                    ...
                                                    <input type="hidden" id="post_id" value="{{ $comment->id }}">
                                                    <button class="btn btn-default" id="delete-comment">Report</button>
                                                    </li>
                                                </ul>
                                        	</span>
											@else
											<?php if($comment->user_id == auth()->user()->id){ ?>
											<span class="dropdown commentoptions" id="">
	                                          		<span  class="border-black"   data-toggle="dropdown">
	                                              		<span   class="glyphicon glyphicon-option-vertical cursor-pointer"></span>
	                                           		 </span>
                                                <ul class="dropdown-menu">
                                                    ...
                                                    <li>
                                                    	{{ csrf_field() }}
                                                     <input type="hidden" id="comment-id-df" value="{{ $comment->id }}">
                                            		<input type="hidden" class ="comment-id" value="0" >
                                                    
                                                    <button class="btn btn-block delete-comment" id="">Delete</button>
                                                    </li>
                                                </ul>
                                        	</span>
											<?php
								...
								<div style="height: 150px;  text-align: ;" class="col-md-7 col-xs-6 border-black">
								 	...
								 	<a href="/video-id-{{ $video->id }}"  class="cursor-pointer ">
									  ...
									  <!-- <div class="panel panel-default" style="width: 100%; float: left; margin-right: 15px;"> -->
								    	<video     preload="metadata" style="border:1px solid silver; width: 100%; height:150px;" poster="/storage/video_thumbnail/{{ $video->user_id }}/{{ $video ->video_thumb }}"   >
										  <source src="/storage/video_posted/{{ $video->user_id }}/{{ $video ->post_video }}" type="video/mp4">
										</video>
									  <!-- </div> -->
								</div>
